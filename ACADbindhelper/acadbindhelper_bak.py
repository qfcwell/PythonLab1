...
restartACSs={'shenzhen':'restartACS_SZ.bat','guangzhou':'restartACS_GZ.bat','shanghai':'restartACS_SH.bat','changsha':'restartACS_CS.bat','hainan':'restartACS_HN.bat'}
...
acs_paths={'shenzhen':'D:\\AcsModule\\Client','guangzhou':'D:\\AcsModule\\Client_GZ','shanghai':'D:\\AcsModule\\Client_SH','changsha':'D:\\AcsModule\\Client_CS','hainan':'D:\\AcsModule\\Client_HN'}
server_conns={
...
def init_helper():
    hostname=socket.gethostname()
    global Server,Method,restartACS,apps,EmptyDwg,exceptions
    while 1:
        print("Getting method for "+hostname)
        time.sleep(1)
        res=GetMethod(hostname)
        if res:
            (Server,Method)=res
            break
    print(Server+str(Method)) 
    restartACS=restartACSs[Server]
    acs_path=acs_paths[Server]
    apps={'CAD':'C:\\Program Files\\Autodesk\\AutoCAD 2012 - Simplified Chinese\\acad.exe','ACS':os.path.join(acs_path,'AcsTools.exe')}
    EmptyDwg=os.path.join(acs_path,'EmptyDwg.dwg')
    exceptions=[
...
def main():
    ...
    log_result(file_name='1',file_path='1',result='1')
...
class BindCleaner():
    ...
    def __init__(self):
        self.lst=[]
        self.sorted_list=[]
        self.not_binded=self.GetNotBinded(server=Server)
        self.f_path=r_path
    ...
    def GetNotBinded(self,server):
        try:
            (host,user,password,database)=server_conns[server]
...
def log_result(file_name,file_path,result):
    try:
        conn=pymysql.connect(host="superset1.minitech.site",user="steven",passwd="steven!@#456",db="project_test")
        cur=conn.cursor()
        hostname=socket.gethostname()
...
def GetMethod(hostname):
    try:
        with pymysql.connect(host="www.minitech.site",user="steven",passwd="steven!@#456",db="project_test") as cur:
            cur.execute(u"SELECT Server,Method FROM BindHelperMethod WHERE ComputerName=%s",[hostname])
            res=cur.fetchone()
            return res
    except:
        return 0
