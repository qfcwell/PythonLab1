...
class acs_project_sync(sync):
    ...
    def get_comp_dept(self,PrjNo):
        cur1=self.oracle.cursor()
        cur1.execute(u"""SELECT PRJ_NO,SAP_PRJNO FROM CAPOL_PRJACSSERVER WHERE PRJ_NO=:1 AND SAP_PRJNO<>:2""",[PrjNo,PrjNo])
        res=cur1.fetchone()
        if res:
            PrjNo=res[1]
        cur=self.cip.cursor()
...
class staff_sync(sync):
    ...
    def get_staff(self):
        cur=self.cip.cursor()
        cur.execute(u"""SELECT LoginName,JobNo,Username,CompanyName,DeptName,Specialty,Specialty as SubDept FROM [dbo].[v_auditor_staff] WHERE JobNo IS NOT NULL and JobNo<>''""")
    ...
    def check_and_insert(self):
        ...
        for (LoginName,JobNo,Username,CompanyName,DeptName,Specialty,SubDept) in staff:
            cur.execute(u"SELECT LoginName,JobNo,Username,CompanyName,DeptName,SubDept FROM CAPOL_CIP_STAFF WHERE LoginName=:1",[LoginName])
            res=cur.fetchone()
            if res:
